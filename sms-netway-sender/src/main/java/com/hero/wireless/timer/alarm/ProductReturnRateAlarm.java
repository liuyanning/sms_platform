...
/**
 * 产品回执率预警
 * 产品回执率预警= （接收失败+接收成功）/提交成功
 * @author gengjinbiao
 * @version V1.0
 * @date 2019年11月10日
 */
@Component
public class ProductReturnRateAlarm extends AlarmBase implements IAlarm {

	@Override
	@PostConstruct
	public boolean probe() {
		List<Alarm> list = getAlarmListByType(AlarmType.PRODUCT_RETURN_RATE_ALARM.toString());
		list.forEach(item -> {
			alarmCheck(item);
		});
		return false;
	...
	public void alarmCheck(Alarm item){
		...
		SCHEDULED_EXECUTOR_SERVICE.scheduleWithFixedDelay(() -> {
			try{
				...
				//组装参数
				String alarmTemplate = DatabaseCache.getAlarmSmsTemplate(AlarmType.PRODUCT_RETURN_RATE_ALARM.toString()+ "_sms_template");
				String recoveryTemplate = DatabaseCache.getAlarmSmsTemplate(AlarmType.PRODUCT_RETURN_RATE_ALARM.toString()+ "_recovery_sms_template");
				String productName = DatabaseCache.getProductByNo(alarmExt.getBind_Value()).getName();
				alarmExt.setAlarm_Result(result);
				alarmExt.setAlarm_Email_Title("产品回执率预警");
				alarmExt.setAlarm_Content(MessageFormat.format(alarmTemplate, productName, alarm.getThreshold_Value()));
				alarmExt.setRecovery_Email_Title("产品回执率恢复通知");
                ...
                logBuf.append("接收"+ defaultString(reportTotal) +"条,");
                logBuf.append("接收成功率 "+(rate)+"%");
                String des = alarmDescription("产品回执率预警",alarmLog
