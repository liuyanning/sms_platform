...
/**
 * 通道回执率预警
 * 通道回执率预警= （接收失败+接收成功）/提交成功
 * @author gengjinbiao
 * @version V1.0
 * @date 2019年11月10日
 */
@Component
public class ChannelReturnRateAlarm extends AlarmBase implements IAlarm {

	@Override
	@PostConstruct
	public boolean probe() {
		List<Alarm> list = getAlarmListByType(AlarmType.CHANNEL_RETURN_RATE_ALARM.toString());
		list.forEach(item -> {
			alarmCheck(item);
		});
		return false;
	...
	public void alarmCheck(Alarm item){
		...
		SCHEDULED_EXECUTOR_SERVICE.scheduleWithFixedDelay(() -> {
			...
			try{
				...
				Integer submitTotal = timerExtDAO.getSubmitSuccessTotal(alarmExt);
                if (submitTotal == null || submitTotal == 0){
                    return;
                }
                Integer reportTotal = timerExtDAO.getReportTotalForReturnRate(alarmExt);
                Double rate = getRate(reportTotal, submitTotal);
				Boolean result = rate < Double.valueOf(alarm.getThreshold_Value());
				//组装参数
				String alarmTemplate = DatabaseCache.getAlarmSmsTemplate(AlarmType.CHANNEL_RETURN_RATE_ALARM.toString()+ "_sms_template");
				String recoveryTemplate = DatabaseCache.getAlarmSmsTemplate(AlarmType.CHANNEL_RETURN_RATE_ALARM.toString()+ "_recovery_sms_template");
                ...
                logBuf.append(",通道提交成功"+defaultString(submitTotal)+"条,");
                logBuf.append("回执"+ defaultString(reportTotal) +"条,");
                logBuf.append("回执率 "+(rate)+"%");
                String des = alarmDescription("通道回执率预警",alarmLog
