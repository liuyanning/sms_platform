...
public class CmppClientCustomHandler extends ICustomHandler {
    ...
    public CmppClientCustomHandler(com.hero.wireless.web.entity.business.Channel bizChannel) {
        this.bizChannel = bizChannel;
    }

    @Override
    public void fireUserLogin(Channel channel, ChannelSession channelSession) {
        logger.debug("cmpp通道【{}】登录成功--开始发送短信", bizChannel.getName());
    }

    @Override
    public void channelClosed(ChannelSession channelSession) {
        GlobalHandler.CMPP_CLIENT_BUSINESS_HANDLER.modifySessionStatus(SessionStatus.DISCONNECT, bizChannel.getNo());
    }
