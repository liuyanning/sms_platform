...
layui.define(["element","laytpl" ,"jquery"], function (exports) {
    ...
    var miniMenu = {
        ...
        compileMenu: function(menu,isSub){
            var menuHtml = '<li {{#if( d.menu){ }}  data-menu="{{d.menu}}" {{#}}} class="layui-nav-item menu-li {{d.childOpenClass}} {{d.className}}"  {{#if( d.id){ }}  id="{{d.id}}" {{#}}}> <a {{#if( d.href){ }} layuimini-href="{{d.href}}?&limitCode={{d.code}}" {{#}}} {{#if( d.target){ }}  target="{{d.target}}" {{#}}} href="javascript:;">{{#if( d.icon){ }}  {{#}}} <span class="layui-left-nav">{{d.title}}</span></a>  {{# if(d.children){}} {{d.children}} {{#}}} </li>' ;
            if(isSub){
                menuHtml = '<dd class="menu-dd {{d.childOpenClass}} {{ d.className }}"> <a href="javascript:;"  {{#if( d.menu){ }}  data-menu="{{d.menu}}" {{#}}} {{#if( d.id){ }}  id="{{d.id}}" {{#}}} {{#if(( !d.child || !d.child.length ) && d.href){ }} layuimini-href="{{d.href}}" {{#}}} {{#if( d.target){ }}  target="{{d.target}}" {{#}}}> {{#if( d.icon){ }}  {{#}}} <span class="layui-left-nav"> {{d.title}}</span></a> {{# if(d.children){}} {{d.children}} {{#}}}</dd>'
            }
        ...
        renderMultiModule: function (menuList, menuChildOpen) {
            ...
            //菜单过多则翻页展示（大于X值 且 水平导航开）
            if($('.layuimini-menu-header-pc li').length>9){
                $('.layuimini-menu-header-pc li:gt(8)').hide();
                $('.layuimini-menu-header-pc li:lt(9)').show();
                //添加 >>
                $('.layuimini-menu-header-pc').append(
                    '<li class="layui-nav-item" lay-unselect>'+
                    '<a href="javascript:;" class="more-menu">更多<i class="fa fa-angle-double-right"></i></a>'+
                    '</li>'
                );
            }
            $('.layuimini-menu-left').html(leftMenuHtml);
        ...
        listen: function () {
             ...
             * 水平导航菜单翻页展示
             */
            $('body').on('click', '.more-menu', function () {
                var text = $('.more-menu').html();
                var sear=new RegExp('fa-angle-double-right');
                if(sear.test(text)){
                    $('.more-menu').html('<i class="fa fa-angle-double-left"></i>返回');
                    $('.layuimini-menu-header-pc li:gt(8)').show();
                    $('.layuimini-menu-header-pc li:lt(9)').hide();
                }else{
                    $('.layuimini-menu-header-pc li:gt(8)').hide();
                    $('.layuimini-menu-header-pc li:lt(9)').show();
                    $('.more-menu').html('更多<i class="fa fa-angle-double-right"></i>').parent().show();
                }
            });
