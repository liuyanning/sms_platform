...
<form class="layui-form" action="" id="subForm"
    ...
    <div class="layui-form-item">
        ...
        <div class="layui-input-block">
            ...
            <button type="button" onclick="deleteCard()" class="layui-btn layui-btn-radius layui-btn-danger" >
                <i class="layui-icon">&#xe640;</i>
                <span>删除卡片</span>
            </button>
        </div>
...
<script >
    ...
    var cardJson = JSON.parse(params['cardJson']);
    var cardIndex = cardJson.cardIndex;
    $("#title").val(cardJson.title);
    $("#description").val(cardJson.description);

    if(cardJson.suggestions){
        for(var i=0;i<cardJson.suggestions.length;i++){
            var menuType = cardJson.suggestions[i].menuType;
            var buttonName = cardJson.suggestions[i].menuName;
            var buttonData = cardJson.suggestions[i].menuValue;
            creatMenuBt(buttonName,menuType,buttonData);
        }
    }
    var picSrc = cardJson.url;
    if(picSrc.IsPicture()){
        $("#mediaShow").attr("onclick","mediaShow('"+picSrc+"','picture')");
    }else {
        picSrc = "/public/admin/images/videoPlayButton.png";
        $("#mediaShow").attr("onclick","mediaShow('"+cardJson.url+"','video')");
    }
    $("#picHuiXian").attr('src', picSrc);
    $("#picHuiXian").attr('width', "100px");
    $("#picHuiXian").attr('height', "100px");
    $("#fileId").val(cardJson.materialNo);
    ...
    //删除卡片
    function deleteCard() {
        var index = layer.confirm('确认要删除该卡片吗？', {
            btn : [ '确定', '取消' ]//按钮
        }, function(index) {
            parent.deleteCard(cardIndex);
        });
    }
    //添加悬浮菜单
    function addOneMenu() {
    ...
    function ajaxSubmitForm() {
        ...
        parent.setCurrentCard(title,description,buttonArray,picSrc,materialNo,cardIndex,url);
        return false;
    }
</script>
</body>